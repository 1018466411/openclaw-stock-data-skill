...
3. **股票历史数据**  
   ...
   - 六个 `get_stock_realtime_*` 工具会自动识别股票、可转债和指数代码；例如 `110070.SH` 可直接传给 `get_stock_realtime_macd`，无需改用可转债工具。除代码外，全部默认参数都可以不传，空字符串和 null 也按默认值处理；可转债历史分钟读取 `convertible_bond_1m`，今天读取 `stock_1m_realtime`
