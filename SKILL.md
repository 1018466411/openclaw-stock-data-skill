...
- **指数与板块接口**：
  ...
  - `get_dc_blocks`：获取 dc 板块列表。
  - `get_dc_daily`：获取 dc 板块日K（按交易日或板块代码）。
  - `get_dc_block_stocks`：获取 dc 板块成分股（支持板块/日期/股票筛选，空参默认最新日期）。
  - `get_tdx_block_stocks`：获取 tdx 板块成分股，返回分页结构 `data.total/page/page_size/list`，其中 `list` 项包含 `block_code`、`block_name`、`block_type`、`stock_code`。
  - `get_tdx_minute`：获取 tdx 板块分钟K线，支持 `1min/5min/15min/30min/60min`，代码兼容 `880201` 和 `880201.TDX`。
...
- **Python便捷函数**：股票、ETF、可转债提供 `get_<market>_macd|mavol|kdj|rsi|boll|ma`。
...
- 股票支持 `adjust=none/qfq`；ETF、可转债当前按不复权行情计算。
...
- Historical-only requests and warmup bars may use Redis caching. If a request range includes today, the API bypasses realtime-bar and calculation-result caches entirely, reads `stock_1m_realtime FINAL`, and recalculates on every request. Convertible-bond history comes from `convertible_bond_1m`, while today's bond bars come from `stock_1m_realtime`. Stock endpoints read today's 09:30 auction bar without caching it for realtime requests.
...
### 19. ths 热度榜：`GET /api/ths/hot` / `POST /api/ths/hot`
